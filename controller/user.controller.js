...
class UserController {
    ...
    try {
      ...
      if (!username) {
        return res.status(StatusCodes.BAD_REQUEST).json(
          errorResponse("Username is required", StatusCodes.BAD_REQUEST)
        );
      }
  ...
  updateUser = async (req, res) => {
    try {
      ...
      if (Object.keys(updates).length === 0) {
        return res.status(StatusCodes.BAD_REQUEST).json(
          errorResponse("Update data is required", StatusCodes.BAD_REQUEST)
        );
      }
  ...
  deleteUser = async (req, res) => {
    try {
      ...
      return res.status(StatusCodes.SUCCESS).json(
        successResponse(user.username, "User deleted successfully")
      );
    } catch (error) {
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
        errorResponse("Failed to delete user", StatusCodes.INTERNAL_SERVER_ERROR)
      );
    }
  };
}
